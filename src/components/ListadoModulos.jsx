...
const ListadoModulos = ({filtro}) => {
    ...
    const obtenerDatos = async () => {
        const data = await fetch('/listado_modulos')
    ...
    return ( 
            ...
            <div className="modulos">
                <div className="row">
                    {
                        modulo.map(item =>
                            <div key={item.id} className={`col-12 col-sm-6 col-lg-3 active px-lg-3 ${item.category}`} data-cat={item.category} data-mod={item.id}>
                                <div className="modulos__modulo modulos__modulo--absolute">
                                    <figure className="modulos__modulo--preview" style={{ backgroundImage: `url(https://bci.qa.globaldigital.cl/salesforce/v2/img/${item.prev_img})`}} />
